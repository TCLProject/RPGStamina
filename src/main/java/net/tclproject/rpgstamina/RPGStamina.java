...
public class RPGStamina
{
    ...
    public void preInit(FMLPreInitializationEvent event)
    {
    	...
    	PatchesConfig.init(event.getModConfigurationDirectory().toString(), event);
    ...
    public void init(FMLInitializationEvent event)
    {
    	...
    	if (PatchesConfig.enableBlazeRodRender) {
    ...
    public void postInit(FMLPostInitializationEvent event) {
    	...
    	if (FMLCommonHandler.instance().getEffectiveSide().isClient()) {
    		MinecraftForge.EVENT_BUS.register(new GuiHealthStamina(Minecraft.getMinecraft()));
    	}
